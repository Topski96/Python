...
# as well as checking if snake touches apple
...
# snake section handles orienting head and drawing a body segment
class SnakeSection:
...
class App:
    def __init__(self):
        pyxel.init(192, 128, title="Pyxel Snake", fps=60, display_scale=8)
        pyxel.load("pyxel_snake.pyxres")
    ...
    def check_collision(self):
        ...
        if self.apple.intersect(self.snake[0].x, self.snake[0].y, self.snake[0].w, self.snake[0].h):
            ...
            if self.apples_eaten_this_level >= 5:
                self.current_level += 1
                self.apples_eaten_this_level = 0
